...
// Remove localStorage for payees and use backend only
let beneficiaries = [];
...
function handleLogin() {
  ...
  // Regular user login via Aadhaar and password
  fetch(`http://localhost:8074/customers/${aadhaar}`)
    .then(res => {
      if (!res.ok) throw new Error("Invalid Aadhaar");
      return res.json();
    ...
    .then(customer => {
      ...
      sessionStorage.setItem("accNo", customer.accountNo); // Store user's account number
    ...
    .catch(err => {
      if (err.message === "Invalid password") return;
      const messageDiv = document.getElementById("login-message");
      showMessage(messageDiv, "Login failed. Please check Aadhaar.", "error");
    });
}
...
function fetchCustomerDetails(aadhaar) {
  fetch(`http://localhost:8074/customers/${aadhaar}`)
    ...
    .then(customer => {
      ...
      div.innerHTML = `
        <h2 class="card-title">Welcome, ${customer.customerName || customer.customername}</h2>
        <div class="card">
          <p><strong>Aadhaar:</strong> ${customer.aadhaar}</p>
          <p><strong>Email:</strong> ${customer.email}</p>
          <p><strong>Mobile:</strong> ${customer.mobile}</p>
          ...
          <p><strong>DOB:</strong> ${customer.dob}</p>
          <p><strong>Gender:</strong> ${customer.gender}</p>
          <p><strong>Marital Status:</strong> ${customer.maritalStatus || customer.maritalstatus}</p>
          <p><strong>Occupation:</strong> ${customer.occupation}</p>
          <p><strong>PAN:</strong> ${customer.pan}</p>
        </div>
      `;
...
// After login, accNo is already stored in sessionStorage as per your last edit
// All payee logic below uses accNo

// Fetch payees for the current user's account number
function fetchPayees() {
  const accNo = sessionStorage.getItem("accNo");
  if (!accNo) return;
  fetch(`http://localhost:8081/payees/${accNo}`)
    .then(res => res.json())
    .then(payees => {
...
// Add a new payee for the current user's account number
function addBeneficiary() {
  ...
  const accNo = sessionStorage.getItem("accNo");
  if (!accNo) {
    showMessage(messageDiv, "User not logged in!", "error");
    return;
  }

  // Check if beneficiary already exists
  const existingBeneficiary = beneficiaries.find(
    (b) => b.payeeAccNo == accountNumber
  );
  ...
  // Prepare payee object as per backend
  const payee = {
    accNo: Number(accNo),
    payeeName: name,
    payeeAccNo: Number(accountNumber)
  };

  fetch("http://localhost:8081/payees", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify(payee)
  })
    .then(res => {
      if (!res.ok) throw new Error("Failed to add payee");
      return res.json();
    })
    .then(() => {
    ...
    .catch(() => {
      showMessage(messageDiv, "Error adding beneficiary!", "error");
    });
}

// Update deleteBeneficiary to use payeeId
function deleteBeneficiary(payeeId) {
  const accNo = sessionStorage.getItem("accNo");
  if (!accNo) return;
  if (confirm("Are you sure you want to delete this beneficiary?")) {
    fetch(`http://localhost:8081/payees/${payeeId}`, { method: "DELETE" })
      .then(res => {
      ...
      .catch(() => {
        showSuccessModal("Error deleting beneficiary!");
      });
  }
}
...
// In populateBeneficiaryDropdowns and updateBeneficiaryList, use payeeName and payeeAccNo
function populateBeneficiaryDropdowns() {
  ...
  dropdowns.forEach((dropdownId) => {
    ...
    beneficiaries.forEach((beneficiary) => {
      ...
      option.value = beneficiary.payeeAccNo;
      const maskedAccount = maskAccountNumber(beneficiary.payeeAccNo);
      option.textContent = `${beneficiary.payeeName} - ${maskedAccount}`;
  ...
  beneficiaries.forEach((beneficiary) => {
    ...
    beneficiaryItem.innerHTML = `
      <div class="beneficiary-info">
        <h4>${beneficiary.payeeName}</h4>
        <p>Account: ${maskedAccount}</p>
      </div>
      <div class="beneficiary-actions">
        <button class="btn-small btn-danger" onclick="deleteBeneficiary('${beneficiary.payeeId}')">Delete</button>
      </div>
    `;
  ...
  const confirmationContent = `
    <div style="text-align: left; margin: 1rem 0;">
      ...
      <div style="background: var(--card-secondary); padding: 1rem; border-radius: 10px; margin-bottom: 1rem;">
        ...
        <p><strong>To Account:</strong> ${selectedBeneficiary.payeeAccNo}</p>
        <p><strong>Beneficiary:</strong> ${selectedBeneficiary.payeeName}${
